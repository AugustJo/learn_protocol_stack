...
int ip_options_compile(struct ip_options * opt, struct sk_buff * skb)
{
	int l;
	unsigned char * iph;
	unsigned char * optptr;	 //optptr[0]:类型    optptr[1]:长度    optptr[2]:指针(相对偏移量)  
	int optlen;		//当前选项的长度
	...
	for (l = opt->optlen; l > 0; ) {
		...
		switch (*optptr) { 
			...
			opt->rr = optptr - iph;   //record route偏移量
			break;
			...
			if (optptr[2] <= optlen) {
				__u32 * timeptr = NULL;
				if (optptr[2]+3 > optptr[1]) {	//选项内容4byte,长度不够返回error
					pp_ptr = optptr + 2;
					goto error;
				}
				switch (optptr[3]&0xF) {  //前4位溢出，后4位子类型
				      case IPOPT_TS_TSONLY:
					opt->ts = optptr - iph;		//ts字段的偏移量
					if (skb) 
						timeptr = (__u32*)&optptr[optptr[2]-1];
					opt->ts_needtime = 1;
					optptr[2] += 4;	//timestamp 4 bytes 
					break;
					...
					opt->ts = optptr - iph;
					if (skb) {	//前4bytes addr,后4bytes timestamp
						memcpy(&optptr[optptr[2]-1], &rt->rt_spec_dst, 4);
						timeptr = (__u32*)& [optptr[2]+3];
					}
					opt->ts_needaddr = 1;
					opt->ts_needtime = 1;
					optptr[2] += 8;
					break;
				      case IPOPT_TS_PRESPEC:	//本地地址才会加timestamp
					if (optptr[2]+7 > optptr[1]) {
						pp_ptr = optptr + 2;
						goto error;
					}
					opt->ts = optptr - iph;
					{
						...
						if (inet_addr_type(addr) == RTN_UNICAST)	//检查是否为本地地址
							break;									//break跳出switch
						if (skb)
